...
class guru extends CI_Controller{
    ...
    var $folder =   "admin/guru";
    var $tables =   "t_rekap_master";
    var $pk     =   "NIP";
    var $title  =   "Guru Pengajar";
    ...
    function index()
    {
		...
		$query  	  =  "SELECT * FROM t_rekap_master WHERE jabatan='Guru' Order by NAMA";
	...
	function post()
    {
        if(isset($_POST['submit']))
        {
			...
			$test= $this->db->insert_batch('personal_guru', $result); 
	...
	function data()
    {
			...
			$id         	 =  $this->uri->segment(4);
			$NIP          	 =  $this->uri->segment(5);
            $ajar            =   " SELECT * FROM personal_guru WHERE nipbaru = '$id' order by id_personal_guru";
